...
- `is_causal=True` is documented to handle non-square query/key lengths correctly ("forms a causal bias for non-square matrices"), treating the query as the tail of the full sequence — this is exactly the semantics needed for a KV-cache decode step (query length 1, key length N) and requires no manual mask.
